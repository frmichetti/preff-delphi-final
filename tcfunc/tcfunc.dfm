object frmtcfunc: Tfrmtcfunc
  Left = 251
  Top = 121
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Tabela de Cadastro de Funcion'#225'rios'
  ClientHeight = 390
  ClientWidth = 610
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Menu = MainMenu1
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object Label2: TLabel
    Left = 8
    Top = 8
    Width = 100
    Height = 15
    Caption = 'Pesquisar por:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Footlight MT Light'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label1: TLabel
    Left = 144
    Top = 8
    Width = 70
    Height = 15
    Caption = 'Descri'#231#227'o'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Footlight MT Light'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object DBGrid1: TDBGrid
    Left = 8
    Top = 56
    Width = 593
    Height = 321
    DataSource = dsfun
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'cod_funcionario'
        Title.Caption = 'C'#243'digo Funcion'#225'rio'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'nome'
        Title.Caption = 'Nome'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'telefone'
        Title.Caption = 'Telefone'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'celular'
        Title.Caption = 'Celular'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'endereco'
        Title.Caption = 'Endere'#231'o'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'bairro'
        Title.Caption = 'Bairro'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'complemento'
        Title.Caption = 'Complemento'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'cidade'
        Title.Caption = 'Cidade'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'estado'
        Title.Caption = 'Estado'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'cep'
        Title.Caption = 'CEP'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'cpf'
        Title.Caption = 'CPF'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'rg'
        Title.Caption = 'RG'
        Visible = True
      end>
  end
  object EditPesq: TEdit
    Left = 144
    Top = 24
    Width = 297
    Height = 21
    Hint = 'Informa'#231#245'es '#224' serem pesquisadas'
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
  end
  object BitBtn1: TBitBtn
    Left = 456
    Top = 16
    Width = 145
    Height = 33
    Hint = 'Procurar'
    Caption = 'Pesquisar'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 3
    OnClick = BitBtn1Click
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00}
    NumGlyphs = 2
  end
  object ComboBox1: TComboBox
    Left = 8
    Top = 24
    Width = 129
    Height = 21
    Hint = 'Selecione uma das op'#231#245'es'
    AutoDropDown = True
    ItemHeight = 13
    ParentShowHint = False
    ShowHint = True
    TabOrder = 2
    Items.Strings = (
      'C'#243'digo'
      'Nome'
      'Telefone'
      'Celular'
      'Endere'#231'o'
      'RG'
      'CPF'
      'CEP'
      '')
  end
  object MainMenu1: TMainMenu
    Left = 920
    object Ajuda1: TMenuItem
      Caption = 'Ajuda'
    end
    object Siar1: TMenuItem
      Caption = 'Sair'
      OnClick = Siar1Click
    end
  end
  object Tfun: TTable
    Active = True
    AutoRefresh = True
    DatabaseName = 'dbpreff'
    TableName = 'dbo.TFuncionario'
    Left = 8
    object Tfuncod_funcionario: TIntegerField
      FieldName = 'cod_funcionario'
      Required = True
    end
    object Tfunnome: TStringField
      FieldName = 'nome'
      Required = True
      Size = 30
    end
    object Tfunendereco: TStringField
      FieldName = 'endereco'
      Required = True
      Size = 50
    end
    object Tfunbairro: TStringField
      FieldName = 'bairro'
      Required = True
      Size = 30
    end
    object Tfuncomplemento: TStringField
      FieldName = 'complemento'
      Size = 50
    end
    object Tfuncidade: TStringField
      FieldName = 'cidade'
      Required = True
      Size = 30
    end
    object Tfunestado: TStringField
      FieldName = 'estado'
      Required = True
      Size = 2
    end
    object Tfuncep: TStringField
      FieldName = 'cep'
      Required = True
      Size = 9
    end
    object Tfuntelefone: TStringField
      FieldName = 'telefone'
      Required = True
      Size = 14
    end
    object Tfuncelular: TStringField
      FieldName = 'celular'
      Size = 14
    end
    object Tfuncpf: TStringField
      FieldName = 'cpf'
      Required = True
      Size = 14
    end
    object Tfunrg: TStringField
      FieldName = 'rg'
      Required = True
      Size = 12
    end
  end
  object dsfun: TDataSource
    DataSet = Tfun
    Left = 760
    Top = 48
  end
  object Queryfun: TQuery
    DatabaseName = 'dbpreff'
    SQL.Strings = (
      'select * '
      'from TFuncionario')
    Left = 840
  end
  object dsqueryfun: TDataSource
    DataSet = Queryfun
    Left = 760
  end
end
